    ...
    <section className="bg-warm-white pt-12 pb-20 md:pt-16 md:pb-28">
      <div className="mx-auto grid w-full max-w-7xl gap-12 px-4 sm:px-6 md:grid-cols-12 md:gap-16 md:px-10">
        ...
        <div className="md:order-1 md:col-span-7">
          ...
          {/* Sized to whatever stats the profile actually carries: an
              unconfirmed fact is dropped from the file rather than shown as a
              dash, so the row must not assume three. */}
          <dl
            className={cn(
              "mt-12 grid gap-6 border-t border-charcoal/15 pt-8",
              profile.stats.length <= 2 ? "grid-cols-2" : "grid-cols-3"
            )}
          >
            {profile.stats.map((stat, index) => (
              ...
              >
                ...
                <dd className="font-display text-3xl text-forest md:text-4xl">{stat.value}</dd>
